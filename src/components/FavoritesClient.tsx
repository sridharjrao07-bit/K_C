...
export default function FavoritesClient({ favoriteArtisans, favoriteProducts }: FavoritesClientProps) {
  const { t } = useLanguage();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 animate-fade-in">
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
            <Link href="/dashboard" className="hover:text-[#6f5c46] transition-colors">{t('favorites.dashboard')}</Link>
            <span>/</span>
            <span className="text-[#6f5c46]">{t('favorites.title')}</span>
          </div>
        ...
        {(favoriteArtisans.length === 0 && favoriteProducts.length === 0) ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-[#e5d1bf] animate-fade-in">
            ...
            <h3 className="text-xl font-bold text-[#6f5c46] mb-2">{t('favorites.noFavorites')}</h3>
          ...
          <div className="space-y-12">
            ...
            {favoriteProducts.length > 0 && (
              <section className="animate-fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {favoriteProducts.map((fav: FavoriteProduct, index: number) => (
                    <div
                      key={fav.id}
                      ...
                      <div className="relative bg-white rounded-2xl shadow-sm border border-[#e5d1bf] overflow-hidden hover:shadow-xl transition-all duration-300">
                        ...
                        <div className="p-4">
                          ...
                          <p className="text-gray-500 text-sm">
                            {t('common.by')} {fav.product.profiles?.full_name || "Unknown"}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            {favoriteArtisans.length > 0 && (
              <section className="animate-fade-in">
                <h2 className="text-2xl font-bold text-[#6f5c46] mb-6 border-b border-[#e5d1bf] pb-2">
                  {t('favorites.favoriteArtisans')}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {favoriteArtisans.map((fav: FavoriteArtisan, index: number) => (
                    <div
                      key={fav.id}
                      className="group animate-slide-up"
                      style={{ animationDelay: `${index * 100}ms` }}
