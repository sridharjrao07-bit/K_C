export type Language = "en" | "hi" | "kn";

export const translations = {
  en: {
    nav: {
      home: "Home",
      about: "About",
      shop: "Shop",
      dashboard: "Dashboard",
      login: "Login",
      getStarted: "Get Started",
      signOut: "Sign Out",
    },
    hero: {
      titlePart1: "Empowering",
      titlePart2: "Traditional Kaarigars",
      description: "Your bridge to the digital marketplace. Get verified, get onboarded, and start selling your authentic handcrafted products.",
      startJourney: "Start Your Journey",
      learnMore: "Learn More",
    },
    features: {
      title: "Why Choose",
      titleAccent: "Kaarigar Connect?",
      subtitle: "We make it easy for traditional artisans to enter the digital marketplace",
      f1_title: "Quick Verification",
      f1_desc: "Get your craft and skills verified by our expert team. We ensure authenticity and quality.",
      f2_title: "Easy Onboarding",
      f2_desc: "Simple step-by-step process to set up your profile, products, and connect to marketplaces.",
      f3_title: "Marketplace Access",
      f3_desc: "Direct access to major e-commerce platforms. Reach millions of customers globally.",
    },
    howItWorks: {
      title: "How It",
      titleAccent: "Works",
      subtitle: "Three simple steps to start selling",
      step1: "Register",
      step1_desc: "Create your account and tell us about your craft and products",
      step2: "Get Verified",
      step2_desc: "Our team verifies your craft and helps you set up your profile",
      step3: "Start Selling",
      step3_desc: "Connect to marketplaces and start reaching customers worldwide",
    },
    cta: {
      title: "Ready to Launch Your Craft?",
      subtitle: "Join thousands of kaarigars who are already selling their products online",
      button: "Get Started Today",
    },
    about: {
      title: "About",
      subtitle: "Bridging the gap between traditional craftsmanship and modern commerce.",
      mission: "Our Mission",
      missionDesc: "Kaarigar Connect was born from a simple observation: India's skilled artisans create beautiful, authentic products, but many lack the digital tools and verification needed to reach larger markets. We're changing that.",
      challenge: "The Challenge",
      challengeDesc: "Traditional artisans often face barriers in digital selling: lack of identity verification, limited digital literacy, no product documentation skills, and difficulty navigating e-commerce platforms.",
      solution: "Our Solution",
      solutionDesc: "We provide end-to-end support: verification of identity and craft, comprehensive training, a curated showcase platform, and tools to prepare for major marketplaces.",
      values: "Our Values",
      respect: "Respect",
      respectDesc: "We honor traditional skills and treat every artisan with dignity and respect.",
      simplicity: "Simplicity",
      simplicityDesc: "Technology should be accessible. We keep things simple and easy to understand.",
      empowerment: "Empowerment",
      empowermentDesc: "We don't just help artisans sell—we teach them to thrive independently.",
    },
    shop: {
      title: "The Artisan Marketplace",
      subtitle: "Discover authentic handcrafted treasures",
      searchPlaceholder: "Search products or artisans...",
      allCategories: "All",
      categories: {
        textiles: "Textiles",
        pottery: "Pottery",
        woodwork: "Woodwork",
        metalwork: "Metalwork",
        jewelry: "Jewelry",
        other: "Other"
      },
      noResults: "No matching products",
      noResultsDesc: "Try adjusting your search or category filter.",
      viewStory: "View Story"
    },
    common: {
      meetArtisans: "Meet Our Master Artisans",
      discoverStories: "Discover the stories and skills of traditional craftsmen from across the country.",
      exploreAll: "Explore All Crafts",
      by: "by",
      inStock: "In Stock",
      soldOut: "Sold Out"
    },
    dashboard: {
      welcome: "Namaste",
      subtitle: "Managing your digital artisan legacy.",
      settings: "Settings",
      viewPublicShop: "View Public Shop",
      verification: "Verification",
      myProducts: "My Products",
      training: "Training",
      startVerification: "Start Verification",
      addNewProduct: "Add New Product",
      startLearning: "Start Learning",
      yourCollection: "Your Collection",
      addAnother: "Add Another",
      editDetails: "Edit Details",
      pending: "Pending",
      active: "Active",
      new: "New",
      noProducts: "You haven't added any products yet.",
      launchFirst: "Launch Your First Product",
      totalEarnings: "Total Earnings",
      storeViews: "Store Views",
      activeProducts: "Active Products",
      pendingItems: "Pending Items"
    },
    auth: {
      signIn: "Sign In",
      welcomeBack: "Welcome Back",
      welcomeSubtitle: "Your craft, your story, your marketplace. Continue your journey with Kaarigar Connect.",
      newTo: "New to Kaarigar Connect?",
      createAccount: "Create an account",
      email: "Email Address",
      password: "Password",
      forgotPassword: "Forgot password?",
      signingIn: "Signing In...",
      placeholderEmail: "you@example.com",
      placeholderPassword: "••••••••",
      resetPassword: "Reset Password",
      sendResetLink: "Send Reset Link",
      backToLogin: "Back to Login",
      updatePassword: "Update Password",
      newPassword: "New Password",
      confirmPassword: "Confirm Password",
      passwordUpdated: "Password updated successfully!",
      checkEmail: "Check your email for the reset link."
    },
    training: {
      title: "Artisan Academy",
      subtitle: "Master the skills needed to succeed in the digital marketplace.",
      backToDashboard: "Back to Dashboard",
      startModule: "Start Module",
      certification: "Kaarigar Certification",
      certSubtitle: "Complete all basic modules to earn a 'Digital Ready' badge on your public profile.",
      modulesComplete: "Modules Complete",
      read: "read",
      modules: {
        "pricing-strategy": {
          title: "Digital Pricing Strategy",
          description: "Learn how to calculate costs, handle taxes, and set competitive prices for an international audience.",
          level: "Essential",
          duration: "10 mins",
          content: "Selling online is different from selling in a local haat. You need to account for hidden costs.\n\n### The Pricing Formula\n**Price = (Materials + Labor + Overhead) × Margin + Shipping + Taxes**\n\n#### 1. Know Your Costs\nDon't just count the fabric or clay. Count the electricity, your packaging time, and the tea you drank!\n\n#### 2. The Global Context\nBuyers in Europe or the US look at quality first. Don't underprice your hard work—it makes it look \"cheap\" instead of \"precious\".\n\n#### 3. Platform Fees\nRemember that marketplaces take a small cut (usually 5-15%). Build this into your final price.",
          tips: ["Never price based on desperation.", "Research similar premium products.", "Offer free shipping by including it in the price."]
        },
        "product-photography": {
          title: "Handmade Photography",
          description: "Capture the soul of your craft using just your smartphone and natural lighting.",
          level: "Intermediate",
          duration: "15 mins",
          content: "Your photos are the only thing your customer can touch before they buy.\n\n### The Three Pillars\n\n#### 1. Light is Everything\nUse natural window light. Avoid direct harsh sun. Early morning or late afternoon light is \"golden\".\n\n#### 2. Clean Backgrounds\nEarthy, neutral backgrounds like light wood, stone, or white linen work best for traditional crafts.\n\n#### 3. The Detail Shot\nBuyers want to see the weave, the brushstroke, and the texture. Take at least one close-up macro shot.",
          tips: ["Clean your phone lens.", "Use a tripod (or a stack of books).", "No flashy filters—keep it real."]
        },
        "digital-storytelling": {
          title: "The Power of Storytelling",
          description: "How to write a bio and product stories that connect emotionally with global buyers.",
          level: "Crucial",
          duration: "12 mins",
          content: "People don't just buy a pot; they buy the hands that made it.\n\n### Your Story Matters\n\n#### 1. The 'Why'\nWhy do you do what you do? Is it a family tradition? A passion you found later in life?\n\n#### 2. The Process\nExplain the difficulty. \"It takes 40 hours to weave one sari.\" This justifies the price.\n\n#### 3. The Impact\nBuying from you supports your village, your family, or a dying art form. Say that proudly.",
          tips: ["Write like you speak.", "Use 'I' and 'My', not 'We' (unless it's a co-op).", "Share your failures too."]
        },
        "shipping-packaging": {
          title: "Safe Shipping & Packaging",
          description: "Design unboxing experiences that protect your craft and wow your customers.",
          level: "Practical",
          duration: "8 mins",
          content: "Broken product = Broken trust.\n\n### Packaging Basics\n\n#### 1. The Drop Test\nPack your item, seal it, and drop it from waist height. If it breaks, pack it better.\n\n#### 2. Eco-Friendly is Better\nInternational buyers love sustainable packaging. Use newspaper, cardboard, or cloth scraps instead of bubble wrap if possible.\n\n#### 3. The Personal Note\nAlways include a handwritten thank you note. It creates a loyal customer for life.",
          tips: ["Double-box fragile items.", "Take a photo of the package before shipping.", "Brand your box with a stamp."]
        },
        "marketplace-verification": {
          title: "Marketplace Verification (Amazon/Flipkart)",
          description: "Step-by-step guide to getting verified on major e-commerce platforms.",
          level: "Advanced",
          duration: "20 mins",
          content: "Ready to go big? Here is how to get on Amazon Karigar and Flipkart Samarth.\n\n### What You Need\n\n#### 1. GST Number\nThis is mandatory for online selling. If you don't have one, apply on the government portal. It is free.\n\n#### 2. Artisan Card / Pehchan Card\nThis government ID proves you are a legitimate artisan. It gives you lower commission rates on these platforms.\n\n#### 3. Bank Account\nMust be in your name or your business name. No joint accounts with non-business partners.\n\n### The Process\n1. Go to Amazon Karigar or Flipkart Samarth seller portal.\n2. Upload your GST and Cancelled Cheque.\n3. Upload your Artisan Card in the 'Certificates' section.\n4. Wait for 48-72 hours for approval.",
          tips: ["Keep your Artisan Card updated.", "Ensure name on Bank Account matches GST exactly.", "Start with 5-10 best products."]
        }
      }
    },
    home: {
      artisanShowcase: {
        title: "Meet Our Master Artisans",
        subtitle: "Discover the stories and skills of traditional craftsmen from across the country.",
        explore: "Explore All Artisans"
      }
    },
    reviews: {
      title: "Customer Reviews",
      writeReview: "Write a Review",
      yourReview: "Your Review",
      noReviews: "No reviews yet. Be the first to review!",
      loginPrompt: "Log in to leave a review"
    },
    favorites: {
      title: "Your Favorites",
      noFavorites: "No favorites yet",
      addedTo: "Added to favorites",
      removedFrom: "Removed from favorites"
    },
    contact: {
      title: "Contact Artisan",
      button: "Contact",
      sendTo: "Send a message to",
      subject: "Subject",
      message: "Message",
      send: "Send Message",
      cancel: "Cancel",
      success: "Message Sent!",
      successDesc: "The artisan will receive your message soon.",
      loginRequired: "Please log in to send a message"
    },
    gallery: {
      title: "Behind the Scenes",
      subtitle: "A glimpse into the creative process",
      manage: "Behind the Scenes Gallery"
    },
    messages: {
      inbox: "Messages",
      noMessages: "No messages yet"
    },
    checkout: {
      title: "Checkout",
      shippingDetails: "Shipping Details",
      fullName: "Full Name",
      phone: "Phone Number",
      address: "Address",
      city: "City",
      state: "State",
      pincode: "Pincode",
      landmark: "Landmark (Optional)",
      checkDelivery: "Check Devlivery",
      placeOrder: "Place Order",
      deliveryAvailable: "Delivery available to this location!",
      deliveryUnavailable: "Sorry, we do not ship to this location yet.",
      orderSuccess: "Order Placed Successfully!",
      orderSuccessDesc: "Thank you for supporting traditional artisans.",
      backToHome: "Back to Home",
      checking: "Checking...",
      processing: "Processing...",
      total: "Total to Pay",
      payNow: "Pay Now",
      secure: "Secure Payment",
      secureCheckout: "Secure Checkout",
      verified: "Verified Artisans",
      orderSummary: "Order Summary",
      subtotal: "Subtotal",
      shipping: "Shipping",
      free: "Free",
    },
    cart: {
      title: "Your Cart",
      empty: "Your cart is empty",
      emptyDesc: "Discover our handcrafted treasures",
      continueShopping: "Continue Shopping",
      proceed: "Proceed to Checkout",
    },
    verification_page: {
      title: "Verify Your Craft",
      whyTitle: "Why we need this?",
      whyDesc: "We want to ensure every product on Kaarigar Connect is 100% handmade by real artisans like you. This protects your hard work from resellers.",
      step1Title: "1. The \"Proof of Work\" Selfie",
      step1Desc: "To prevent scammers, we need to see you with your craft.",
      takePhoto: "Take a Selfie",
      clickUpload: "Click to upload or take photo",
      step2Title: "2. Identity Proof",
      step2Desc: "Upload your Artisan Card, Aadhar, or Government ID.",
      uploadId: "Upload ID Proof",
      uploadDoc: "Upload document (JPG, PNG, PDF)",
      step3Title: "3. Work Samples",
      step3Desc: "Upload 2-3 photos of your recent work.",
      uploadSamples: "Upload Work Samples",
      selectPhotos: "Select photos",
      filesSelected: "files selected",
      step4Title: "4. Business & Banking",
      step4Desc: "Required for receiving payments directly to your account.",
      labelGST: "GST Number (Optional)",
      labelAccountName: "Account Holder Name",
      labelAccountNumber: "Bank Account Number",
      labelIFSC: "IFSC Code",
      labelBankProof: "Cancelled Cheque / Passbook",
      uploadBankProof: "Upload Bank Proof",
      uploading: "Uploading...",
      submit: "Submit Verification",
      success: "Verification submitted successfully!",
      alertId: "Please upload ID proof",
      alertSelfie: "Please upload a selfie",
      alertBank: "Please complete banking details"
    },
    action: "Action",
    admin: {
      title: "Admin Dashboard",
      pendingRequests: "Pending Verifications",
      noRequests: "No pending requests.",
      approve: "Approve",
      reject: "Reject",
      viewProof: "View Proofs",
      gst: "GST",
      bank: "Banking",
      status: "Status",
      action: "Action"
    }
  },
  hi: {
    nav: {
      home: "होम",
      about: "हमारे बारे में",
      shop: "दुकान",
      dashboard: "डैशबोर्ड",
      login: "लॉगिन",
      getStarted: "शुरू करें",
      signOut: "साइन आउट",
    },
    hero: {
      titlePart1: "सशक्त बनाते",
      titlePart2: "पारंपरिक कारीगर",
      description: "डिजिटल मार्केटप्लेस के लिए आपका सेतु। सत्यापित हों, ऑनबोर्ड हों और अपने प्रामाणिक हस्तनिर्मित उत्पादों को बेचना शुरू करें।",
      startJourney: "अपनी यात्रा शुरू करें",
      learnMore: "अधिक जानें",
    },
    features: {
      title: "क्यों चुनें",
      titleAccent: "कारीगर कनेक्ट?",
      subtitle: "हम पारंपरिक कारीगरों के लिए डिजिटल मार्केटप्लेस में प्रवेश करना आसान बनाते हैं",
      f1_title: "त्वरित सत्यापन",
      f1_desc: "हमारी विशेषज्ञ टीम द्वारा अपने शिल्प और कौशल का सत्यापन कराएं। हम प्रामाणिकता और गुणवत्ता सुनिश्चित करते हैं।",
      f2_title: "आसान ऑनबोर्डिंग",
      f2_desc: "अपनी प्रोफ़ाइल, उत्पाद सेट करने और मार्केटप्लेस से जुड़ने के लिए सरल चरण-दर-चरण प्रक्रिया।",
      f3_title: "मार्केटप्लेस एक्सेस",
      f3_desc: "प्रमुख ई-कॉमर्स प्लेटफॉर्म तक सीधी पहुंच। विश्व स्तर पर लाखों ग्राहकों तक पहुंचें।",
    },
    howItWorks: {
      title: "यह कैसे",
      titleAccent: "काम करता है",
      subtitle: "बेचना शुरू करने के लिए तीन सरल कदम",
      step1: "पंजीकरण",
      step1_desc: "अपना खाता बनाएं और हमें अपने शिल्प और उत्पादों के बारे में बताएं",
      step2: "सत्यापित हों",
      step2_desc: "हमारी टीम आपके शिल्प को सत्यापित करती है और आपकी प्रोफ़ाइल सेट करने में आपकी सहायता करती है",
      step3: "बेचना शुरू करें",
      step3_desc: "मार्केटप्लेस से जुड़ें और दुनिया भर के ग्राहकों तक पहुंचना शुरू करें",
    },
    cta: {
      title: "क्या आप अपना शिल्प लॉन्च करने के लिए तैयार हैं?",
      subtitle: "उन हजारों कारीगरों से जुड़ें जो पहले से ही अपने उत्पाद ऑनलाइन बेच रहे हैं",
      button: "आज ही शुरू करें",
    },
    about: {
      title: "हमारे बारे में",
      subtitle: "पारंपरिक शिल्प कौशल और आधुनिक वाणिज्य के बीच की दूरी को पाटना।",
      mission: "हमारा लक्ष्य",
      missionDesc: "करीगर कनेक्ट का जन्म एक सरल अवलोकन से हुआ था: भारत के कुशल कारीगर सुंदर, प्रामाणिक उत्पाद बनाते हैं, लेकिन कई के पास बड़े बाजारों तक पहुँचने के लिए आवश्यक डिजिटल उपकरण और सत्यापन की कमी होती है। हम उसे बदल रहे हैं।",
      challenge: "चुनौती",
      challengeDesc: "पारंपरिक कारीगरों को अक्सर डिजिटल बिक्री में बाधाओं का सामना करना पड़ता है: पहचान सत्यापन की कमी, सीमित डिजिटल साक्षरता, उत्पाद प्रलेखन कौशल की कमी, और ई-कॉमर्स प्लेटफॉर्म को समझने में कठिनाई।",
      solution: "हमारा समाधान",
      solutionDesc: "हम शुरू से अंत तक सहायता प्रदान करते हैं: पहचान और शिल्प का सत्यापन, व्यापक प्रशिक्षण, एक क्यूरेटेड शोकेस प्लेटफॉर्म, और प्रमुख बाजारों के लिए तैयार होने के लिए उपकरण।",
      values: "हमारे आदर्श",
      respect: "सम्मान",
      respectDesc: "हम पारंपरिक कौशल का सम्मान करते हैं और प्रत्येक कारीगर के साथ सम्मान और गरिमा के साथ व्यवहार करते हैं।",
      simplicity: "सरलता",
      simplicityDesc: "तकनीक सुलभ होनी चाहिए। हम चीजों को सरल और समझने में आसान रखते हैं।",
      empowerment: "सशक्तिकरण",
      empowermentDesc: "हम केवल कारीगरों को बेचने में मदद नहीं करते—हम उन्हें स्वतंत्र रूप से फलने-फूलने के लिए सिखाते हैं।",
    },
    shop: {
      title: "कारीगर मार्केटप्लेस",
      subtitle: "प्रामाणिक हस्तशिल्प खजाने की खोज करें",
      searchPlaceholder: "उत्पाद या कारीगर खोजें...",
      allCategories: "सभी",
      categories: {
        textiles: "कपड़े",
        pottery: "मिट्टी के बर्तन",
        woodwork: "लकड़ी का काम",
        metalwork: "धातु का काम",
        jewelry: "आभूषण",
        other: "अन्य"
      },
      noResults: "कोई मेल खाने वाले उत्पाद नहीं",
      noResultsDesc: "अपनी खोज या श्रेणी फ़िल्टर को समायोजित करने का प्रयास करें।",
      viewStory: "कहानी देखें"
    },
    common: {
      meetArtisans: "हमारे मास्टर कारीगरों से मिलें",
      discoverStories: "देश भर के पारंपरिक शिल्पकारों की कहानियों और कौशल की खोज करें।",
      exploreAll: "सभी शिल्पों का अन्वेषण करें",
      by: "द्वारा",
      inStock: "स्टॉक में",
      soldOut: "बिक गया"
    },
    dashboard: {
      welcome: "नमस्ते",
      subtitle: "अपनी डिजिटल कारीगर विरासत का प्रबंधन करें।",
      settings: "सेटिंग्स",
      viewPublicShop: "पब्लिक शॉप देखें",
      verification: "सत्यापन",
      myProducts: "मेरे उत्पाद",
      training: "प्रशिक्षण",
      startVerification: "सत्यापन शुरू करें",
      addNewProduct: "नया उत्पाद जोड़ें",
      startLearning: "सीखना शुरू करें",
      yourCollection: "आपका संग्रह",
      addAnother: "एक और जोड़ें",
      editDetails: "विवरण संपादित करें",
      pending: "लंबित",
      active: "सक्रिय",
      new: "नया",
      noProducts: "आपने अभी तक कोई उत्पाद नहीं जोड़ा है।",
      launchFirst: "अपना पहला उत्पाद लॉन्च करें",
      totalEarnings: "कुल कमाई",
      storeViews: "स्टोर व्यूज",
      activeProducts: "सक्रिय उत्पाद",
      pendingItems: "लंबित आइटम"
    },
    auth: {
      signIn: "साइन इन करें",
      welcomeBack: "वापसी पर स्वागत है",
      welcomeSubtitle: "आपका शिल्प, आपकी कहानी, आपका बाजार। कारीगर कनेक्ट के साथ अपनी यात्रा जारी रखें।",
      newTo: "कारीगर कनेक्ट पर नए हैं?",
      createAccount: "खाता बनाएं",
      email: "ईमेल पता",
      password: "पासवर्ड",
      forgotPassword: "पासवर्ड भूल गए?",
      signingIn: "साइन इन हो रहा है...",
      placeholderEmail: "aap@example.com",
      placeholderPassword: "••••••••",
      resetPassword: "पासवर्ड रीसेट करें",
      sendResetLink: "रीसेट लिंक भेजें",
      backToLogin: "लॉगिन पर वापस जाएं",
      updatePassword: "पासवर्ड अपडेट करें",
      newPassword: "नया पासवर्ड",
      confirmPassword: "पासवर्ड की पुष्टि करें",
      passwordUpdated: "पासवर्ड सफलतापूर्वक अपडेट किया गया!",
      checkEmail: "रीसेट लिंक के लिए अपना ईमेल देखें।",
    },
    cart: {
      title: "आपकी टोकरी",
      empty: "आपकी टोकरी खाली है",
      emptyDesc: "हमारे हस्तनिर्मित खजाने की खोज करें",
      continueShopping: "खरीदारी जारी रखें",
      proceed: "चेकआउट के लिए आगे बढ़ें",
    },
    training: {
      title: "कारीगर अकादमी",
      subtitle: "डिजिटल मार्केटप्लेस में सफल होने के लिए आवश्यक कौशल में महारत हासिल करें।",
      backToDashboard: "डैशबोर्ड पर वापस जाएं",
      startModule: "मॉड्यूल शुरू करें",
      certification: "कारीगर प्रमाणन",
      certSubtitle: "अपने सार्वजनिक प्रोफाइल पर 'डिजिटल रेडी' बैज अर्जित करने के लिए सभी बुनियादी मॉड्यूल पूरे करें।",
      modulesComplete: "मॉड्यूल पूरे हुए",
      read: "पढ़ें",
      modules: {
        "pricing-strategy": {
          title: "डिजिटल मूल्य निर्धारण रणनीति",
          description: "लागत की गणना करना, करों को संभालना और अंतरराष्ट्रीय दर्शकों के लिए प्रतिस्पर्धी मूल्य निर्धारित करना सीखें।",
          level: "आवश्यक",
          duration: "10 मिनट",
          content: "ऑनलाइन बेचना स्थानीय हाट में बेचने से अलग है। आपको छिपी हुई लागतों का हिसाब देना होगा।\n\n### मूल्य निर्धारण का सूत्र\n**मूल्य = (सामग्री + श्रम + उपरिव्यय) × मार्जिन + शिपिंग + कर**\n\n#### 1. अपनी लागत जानें\nकेवल कपड़े या मिट्टी की गिनती न करें। बिजली, अपने पैकेजिंग समय और आपके द्वारा पी गई चाय की भी गिनती करें!\n\n#### 2. वैश्विक संदर्भ\nयूरोप या अमेरिका के खरीदार सबसे पहले गुणवत्ता देखते हैं। अपनी मेहनत की कीमत कम न आँकें—इससे यह \"कीमती\" के बजाय \"सस्ता\" दिखता है।\n\n#### 3. प्लेटफ़ॉर्म शुल्क\nयाद रखें कि बाज़ार एक छोटा हिस्सा लेते हैं (आमतौर पर 5-15%)। इसे अपनी अंतिम कीमत में शामिल करें।",
          tips: ["कभी भी हताशा के आधार पर कीमत न तय करें।", "समान प्रीमियम उत्पादों पर शोध करें।", "निःशुल्क शिपिंग की पेशकश करें और इसे कीमत में शामिल करें।"]
        },
        "product-photography": {
          title: "हस्तशिल्प फोटोग्राफी",
          description: "केवल अपने स्मार्टफोन और प्राकृतिक रोशनी का उपयोग करके अपने शिल्प की आत्मा को कैद करें।",
          level: "मध्यम",
          duration: "15 मिनट",
          content: "आपकी तस्वीरें एकमात्र ऐसी चीज हैं जिसे खरीदने से पहले आपका ग्राहक छू सकता है।\n\n### तीन स्तंभ\n\n#### 1. रोशनी ही सब कुछ है\nखिड़की की प्राकृतिक रोशनी का प्रयोग करें। सीधी तेज धूप से बचें। सुबह-सुबह या देर दोपहर की रोशनी \"सुनहरी\" होती है।\n\n#### 2. साफ पृष्ठभूमि\nहल्की लकड़ी, पत्थर, या सफेद लिनन जैसी मिट्टी की, तटस्थ पृष्ठभूमि पारंपरिक शिल्प के लिए सबसे अच्छा काम करती है।\n\n#### 3. विवरण शॉट\nखरीदार बुनाई, ब्रशस्ट्रोक और बनावट देखना चाहते हैं। कम से कम एक क्लोज़-अप मैक्रो शॉट लें।",
          tips: ["अपने फोन के लेंस को साफ करें।", "ट्राइपॉड (या किताबों के ढेर) का उपयोग करें।", "कोई आकर्षक फिल्टर नहीं—इसे असली रखें।"]
        },
        "digital-storytelling": {
          title: "कहानी सुनाने की शक्ति",
          description: "वैश्विक खरीदारों के साथ भावनात्मक रूप से जुड़ने के लिए एक बायो और उत्पाद की कहानियां कैसे लिखें।",
          level: "महत्वपूर्ण",
          duration: "12 मिनट",
          content: "लोग सिर्फ बर्तन नहीं खरीदते; वे उन हाथों को खरीदते हैं जिन्होंने इसे बनाया है।\n\n### आपकी कहानी मायने रखती है\n\n#### 1. 'क्यों'\nआप जो करते हैं वह क्यों करते हैं? क्या यह एक पारिवारिक परंपरा है? एक जुनून जो आपने बाद में जीवन में पाया?\n\n#### 2. प्रक्रिया\nकठिनाई बताएं। \"एक साड़ी को बुनने में 40 घंटे लगते हैं।\" यह कीमत को सही ठहराता है।\n\n#### 3. प्रभाव\nआपसे खरीदना आपके गांव, आपके परिवार या मरती हुई कला का समर्थन करता है। इसे गर्व से कहें।",
          tips: ["जैसे आप बोलते हैं वैसे लिखें।", "'मैं' और 'मेरा' का प्रयोग करें, 'हम' का नहीं (जब तक कि यह सहकारी न हो)।", "अपनी असफलताओं को भी साझा करें।"]
        },
        "shipping-packaging": {
          title: "सुरक्षित शिपिंग और पैकेजिंग",
          description: "अनबॉक्सिंग अनुभव डिज़ाइन करें जो आपके शिल्प की सुरक्षा करें और आपके ग्राहकों को वाह कर दें।",
          level: "व्यावहारिक",
          duration: "8 मिनट",
          content: "टूटा हुआ उत्पाद = टूटा हुआ विश्वास।\n\n### पैकेजिंग मूल बातें\n\n#### 1. ड्रॉप टेस्ट\nअपने आइटम को पैक करें, इसे सील करें, और इसे कमर की ऊंचाई से गिराएं। अगर यह टूट जाता है, तो इसे बेहतर पैक करें।\n\n#### 2. पर्यावरण के अनुकूल बेहतर है\nअंतरराष्ट्रीय खरीदार टिकाऊ पैकेजिंग पसंद करते हैं। यदि संभव हो तो बबल रैप के बजाय समाचार पत्र, कार्डबोर्ड या कपड़े के टुकड़ों का उपयोग करें।\n\n#### 3. व्यक्तिगत नोट\nहमेशा एक हस्तलिखित धन्यवाद नोट शामिल करें। यह जीवन भर के लिए एक वफादार ग्राहक बनाता है।",
          tips: ["नाजुक वस्तुओं को डबल-बॉक्स करें।", "शिपिंग से पहले पैकेज की एक फोटो लें।", "अपने बॉक्स को स्टैम्प के साथ ब्रांड करें।"]
        },
        "marketplace-verification": {
          title: "मार्केटप्लेस सत्यापन (Amazon/Flipkart)",
          description: "प्रमुख ई-कॉमर्स प्लेटफॉर्म पर सत्यापित होने के लिए चरण-दर-चरण मार्गदर्शिका।",
          level: "उन्नत",
          duration: "20 मिनट",
          content: "बड़ा करने के लिए तैयार हैं? यहां बताया गया है कि Amazon Karigar और Flipkart Samarth पर कैसे आएं।\n\n### आपको क्या चाहिए\n\n#### 1. GST नंबर\nऑनलाइन बिक्री के लिए यह अनिवार्य है। यदि आपके पास यह नहीं है, तो सरकारी पोर्टल पर आवेदन करें। यह मुफ़्त है।\n\n#### 2. कारीगर कार्ड / पहचान कार्ड\nयह सरकारी आईडी साबित करती है कि आप एक वैध कारीगर हैं। यह आपको इन प्लेटफॉर्म पर कम कमीशन दरें देता है।\n\n#### 3. बैंक खाता\nआपके नाम या आपके व्यवसाय के नाम पर होना चाहिए। गैर-व्यावसायिक भागीदारों के साथ कोई संयुक्त खाते नहीं।\n\n### प्रक्रिया\n1. Amazon Karigar या Flipkart Samarth विक्रेता पोर्टल पर जाएं।\n2. अपना GST और रद्द चेक अपलोड करें।\n3. 'प्रमाणपत्र' अनुभाग में अपना कारीगर कार्ड अपलोड करें।\n4. अनुमोदन के लिए 48-72 घंटे प्रतीक्षा करें।",
          tips: ["अपना कारीगर कार्ड अपडेट रखें।", "सुनिश्चित करें कि बैंक खाते पर नाम बिल्कुल GST से मेल खाता है।", "5-10 सर्वश्रेष्ठ उत्पादों के साथ शुरुआत करें।"]
        }
      }
    },
    home: {
      artisanShowcase: {
        title: "हमारे मास्टर कारीगरों से मिलें",
        subtitle: "देश भर के पारंपरिक शिल्पकारों की कहानियों और कौशल की खोज करें।",
        explore: "सभी कारीगरों का अन्वेषण करें"
      }
    },
    reviews: {
      title: "ग्राहक समीक्षाएं",
      writeReview: "समीक्षा लिखें",
      yourReview: "आपकी समीक्षा",
      noReviews: "अभी तक कोई समीक्षा नहीं। पहले समीक्षक बनें!",
      loginPrompt: "समीक्षा देने के लिए लॉगिन करें"
    },
    favorites: {
      title: "आपके पसंदीदा",
      noFavorites: "अभी तक कोई पसंदीदा नहीं",
      addedTo: "पसंदीदा में जोड़ा गया",
      removedFrom: "पसंदीदा से हटाया गया"
    },
    contact: {
      title: "कारीगर से संपर्क करें",
      button: "संपर्क करें",
      sendTo: "संदेश भेजें",
      subject: "विषय",
      message: "संदेश",
      send: "संदेश भेजें",
      cancel: "रद्द करें",
      success: "संदेश भेजा गया!",
      successDesc: "कारीगर को जल्द ही आपका संदेश मिलेगा।",
      loginRequired: "संदेश भेजने के लिए लॉगिन करें"
    },
    gallery: {
      title: "बनने की कहानी",
      subtitle: "रचनात्मक प्रक्रिया की एक झलक",
      manage: "बनने की कहानी गैलरी"
    },
    messages: {
      inbox: "संदेश",
      noMessages: "अभी तक कोई संदेश नहीं"
    },
    verification_page: {
      title: "अपने शिल्प का सत्यापन करें",
      whyTitle: "हमें इसकी आवश्यकता क्यों है?",
      whyDesc: "हम यह सुनिश्चित करना चाहते हैं कि कारीगर कनेक्ट पर हर उत्पाद 100% आपके जैसे असली कारीगरों द्वारा हस्तनिर्मित है। यह आपकी मेहनत को पुनर्विक्रेताओं (resellers) से बचाता है।",
      step1Title: "1. \"प्रूफ ऑफ वर्क\" सेल्फी",
      step1Desc: "घोटालेबाजों को रोकने के लिए, हमें आपको आपके शिल्प के साथ देखना होगा।",
      takePhoto: "सेल्फी लें",
      clickUpload: "अपलोड करने या फोटो लेने के लिए क्लिक करें",
      step2Title: "2. पहचान प्रमाण",
      step2Desc: "अपना कारीगर कार्ड, आधार, या सरकारी आईडी अपलोड करें।",
      uploadId: "आईडी प्रूफ अपलोड करें",
      uploadDoc: "दस्तावेज़ अपलोड करें (JPG, PNG, PDF)",
      step3Title: "3. काम के नमूने",
      step3Desc: "अपने हालिया काम की 2-3 तस्वीरें अपलोड करें।",
      uploadSamples: "काम के नमूने अपलोड करें",
      selectPhotos: "तस्वीरें चुनें",
      filesSelected: "फाइलें चुनी गईं",
      step4Title: "4. व्यापार और बैंकिंग",
      step4Desc: "सीधे अपने खाते में भुगतान प्राप्त करने के लिए आवश्यक है।",
      labelGST: "GST नंबर (वैकल्पिक)",
      labelAccountName: "खाता धारक का नाम",
      labelAccountNumber: "बैंक खाता संख्या",
      labelIFSC: "IFSC कोड",
      labelBankProof: "रद्द चेक / पासबुक",
      uploadBankProof: "बैंक प्रमाण अपलोड करें",
      uploading: "अपलोड हो रहा है...",
      submit: "सत्यापन जमा करें",
      success: "सत्यापन सफलतापूर्वक जमा किया गया!",
      alertId: "कृपया आईडी प्रूफ अपलोड करें",
      alertSelfie: "कृपया एक सेल्फी अपलोड करें",
      alertBank: "कृपया बैंकिंग विवरण पूरा करें"
    },
    admin: {
      title: "एडमिन डैशबोर्ड",
      pendingRequests: "लंबित सत्यापन",
      noRequests: "कोई लंबित अनुरोध नहीं।",
      approve: "स्वीकार करें",
      reject: "अस्वीकार करें",
      viewProof: "प्रमाण देखें",
      gst: "GST",
      bank: "बैंकिंग",
      status: "स्थिति",
      action: "कार्रवाई"
    },
    checkout: {
      title: "चेकआउट",
      shippingDetails: "शिपिंग विवरण",
      fullName: "पूरा नाम",
      phone: "फ़ोन नंबर",
      address: "पता",
      city: "शहर",
      state: "राज्य",
      pincode: "पिनकोड",
      landmark: "लैंडमार्क (वैकल्पिक)",
      checkDelivery: "डिलीवरी की जाँच करें",
      placeOrder: "ऑर्डर करें",
      deliveryAvailable: "इस स्थान पर डिलीवरी उपलब्ध है!",
      deliveryUnavailable: "क्षमा करें, हम अभी इस स्थान पर शिप नहीं करते हैं।",
      orderSuccess: "ऑर्डर सफलतापूर्वक दिया गया!",
      orderSuccessDesc: "पारंपरिक कारीगरों का समर्थन करने के लिए धन्यवाद।",
      backToHome: "होम पर वापस जाएं",
      checking: "जाँच हो रही है...",
      processing: "प्रक्रिया हो रही है...",
      total: "कुल भुगतान",
      secure: "सुरक्षित भुगतान",
      secureCheckout: "सुरक्षित चेकआउट",
      verified: "सत्यापित कारीगर",
      orderSummary: "ऑर्डर सारांश",
      subtotal: "उपयोग",
      shipping: "शिपिंग",
      free: "मुफ़्त",
    },
  },
  kn: {
    nav: {
      home: "ಮನೆ",
      about: "ನಮ್ಮ ಬಗ್ಗೆ",
      shop: "ಅಂಗಡಿ",
      dashboard: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
      login: "ಲಾಗಿನ್",
      getStarted: "ಪ್ರಾರಂಭಿಸಿ",
      signOut: "ಸೈನ್ ಔಟ್",
    },
    hero: {
      titlePart1: "ಸಬಲೀಕರಣ",
      titlePart2: "ಸಾಂಪ್ರದಾಯಿಕ ಕುಶಲಕರ್ಮಿಗಳು",
      description: "ಡಿಜಿಟಲ್ ಮಾರುಕಟ್ಟೆಗೆ ನಿಮ್ಮ ಸೇತುವೆ. ಪರಿಶೀಲಿಸಿಕೊಳ್ಳಿ, ನೋಂದಾಯಿಸಿಕೊಳ್ಳಿ ಮತ್ತು ನಿಮ್ಮ ಅಧಿಕೃತ ಕರಕುಶಲ ಉತ್ಪನ್ನಗಳನ್ನು ಮಾರಾಟ ಮಾಡಲು ಪ್ರಾರಂಭಿಸಿ.",
      startJourney: "ನಿಮ್ಮ ಪ್ರಯಾಣ ಪ್ರಾರಂಭಿಸಿ",
      learnMore: "ಇನ್ನಷ್ಟು ತಿಳಿಯಿರಿ",
    },
    features: {
      title: "ೇಕೆ ಆಯ್ಕೆ ಮಾಡಬೇಕು",
      titleAccent: "ಕುಶಲಕರ್ಮಿ ಕನೆಕ್ಟ್?",
      subtitle: "ಸಾಂಪ್ರದಾಯಿಕ ಕುಶಲಕರ್ಮಿಗಳಿಗೆ ಡಿಜಿಟಲ್ ಮಾರುಕಟ್ಟೆಯನ್ನು ಪ್ರವೇಶಿಸುವುದನ್ನು ನಾವು ಸುಲಭಗೊಳಿಸುತ್ತೇವೆ",
      f1_title: "ತ್ವರಿತ ಪರಿಶೀಲನೆ",
      f1_desc: "ನಮ್ಮ ತಜ್ಞರ ತಂಡದಿಂದ ನಿಮ್ಮ ಕಲೆ ಮತ್ತು ಕೌಶಲ್ಯಗಳನ್ನು ಪರಿಶೀಲಿಸಿಕೊಳ್ಳಿ. ನಾವು ಅಧಿಕೃತತೆ ಮತ್ತು ಗುಣಮಟ್ಟವನ್ನು ಖಚಿತಪಡಿಸುತ್ತೇವೆ.",
      f2_title: "ಸುಲಭ ನೋಂದಣಿ",
      f2_desc: "ನಿಮ್ಮ ಪ್ರೊಫೈಲ್, ಉತ್ಪನ್ನಗಳನ್ನು ಹೊಂದಿಸಲು ಮತ್ತು ಮಾರುಕಟ್ಟೆಗಳಿಗೆ ಸಂಪರ್ಕಿಸಲು ಸರಳ ಹಂತ-ಹಂತದ ಪ್ರಕ್ರಿಯೆ.",
      f3_title: "ಮಾರುಕಟ್ಟೆ ಪ್ರವೇಶ",
      f3_desc: "ಪ್ರಮುಖ ಇ-ಕಾಮರ್ಸ್ ವೇದಿಕೆಗಳಿಗೆ ನೇರ ಪ್ರವೇಶ. ಜಾಗತಿಕವಾಗಿ ಲಕ್ಷಾಂತರ ಗ್ರಾಹಕರನ್ನು ತಲುಪಿ.",
    },
    howItWorks: {
      title: "ಇದು ಹೇಗೆ",
      titleAccent: "ಕೆಲಸ ಮಾಡುತ್ತದೆ",
      subtitle: "ಮಾರಾಟ ಪ್ರಾರಂಭಿಸಲು ಮೂರು ಸರಳ ಹಂತಗಳು",
      step1: "ನೋಂದಣಿ",
      step1_desc: "ನಿಮ್ಮ ಖಾತೆಯನ್ನು ರಚಿಸಿ ಮತ್ತು ನಿಮ್ಮ ಕಲೆ ಮತ್ತು ಉತ್ಪನ್ನಗಳ ಬಗ್ಗೆ ನಮಗೆ ತಿಳಿಸಿ",
      step2: "ಪರಿಶೀಲಿಸಿಕೊಳ್ಳಿ",
      step2_desc: "ನಮ್ಮ ತಂಡವು ನಿಮ್ಮ ಕಲೆಯನ್ನು ಪರಿಶೀಲಿಸುತ್ತದೆ ಮತ್ತು ನಿಮ್ಮ ಪ್ರೊಫೈಲ್ ಹೊಂದಿಸಲು ಸಹಾಯ ಮಾಡುತ್ತದೆ",
      step3: "ಮಾರಾಟ ಪ್ರಾರಂಭಿಸಿ",
      step3_desc: "ಮಾರುಕಟ್ಟೆಗಳಿಗೆ ಸಂಪರ್ಕಿಸಿ ಮತ್ತು ವಿಶ್ವದಾದ್ಯಂತ ಗ್ರಾಹಕರನ್ನು ತಲುಪಲು ಪ್ರಾರಂಭಿಸಿ",
    },
    cta: {
      title: "ನಿಮ್ಮ ಕಲೆಯನ್ನು ಪ್ರಾರಂಭಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ?",
      subtitle: "ಈಗಾಗಲೇ ತಮ್ಮ ಉತ್ಪನ್ನಗಳನ್ನು ಆನ್‌ಲೈನ್‌ನಲ್ಲಿ ಮಾರಾಟ ಮಾಡುತ್ತಿರುವ ಸಾವಿರಾರು ಕುಶಲಕರ್ಮಿಗಳೊಂದಿಗೆ ಸೇರಿ",
      button: "ಇಂದೇ ಪ್ರಾರಂಭಿಸಿ",
    },
    about: {
      title: "ನಮ್ಮ ಬಗ್ಗೆ",
      subtitle: "ಸಾಂಪ್ರದಾಯಿಕ ಕರಕುಶಲತೆ ಮತ್ತು ಆಧುನಿಕ ವಾಣಿಜ್ಯದ ನಡುವಿನ ಅಂತರವನ್ನು ಕಡಿಮೆ ಮಾಡುವುದು.",
      mission: "ನಮ್ಮ ಗುರಿ",
      missionDesc: "ಭಾರತದ ನುರಿತ ಕುಶಲಕರ್ಮಿಗಳು ಸುಂದರವಾದ, ಅಧಿಕೃತ ಉತ್ಪನ್ನಗಳನ್ನು ತಯಾರಿಸುತ್ತಾರೆ, ಆದರೆ ಅನೇಕರಿಗೆ ದೊಡ್ಡ ಮಾರುಕಟ್ಟೆಗಳನ್ನು ತಲುಪಲು ಅಗತ್ಯವಾದ ಡಿಜಿಟಲ್ ಪರಿಕರಗಳು ಮತ್ತು ಪರಿಶೀಲನೆಯ ಕೊರತೆಯಿದೆ ಎಂಬ ಸರಳ ಅವಲೋಕನದಿಂದ ಕುಶಲಕರ್ಮಿ ಕನೆಕ್ಟ್ ಜನಿಸಿತು. ನಾವು ಅದನ್ನು ಬದಲಾಯಿಸುತ್ತಿದ್ದೇವೆ.",
      challenge: "ಸವಾಲು",
      challengeDesc: "ಸಾಂಪ್ರದಾಯಿಕ ಕುಶಲಕರ್ಮಿಗಳು ಡಿಜಿಟಲ್ ಮಾರಾಟದಲ್ಲಿ ಹೆಚ್ಚಾಗಿ ಅಡೆತಡೆಗಳನ್ನು ಎದುರಿಸುತ್ತಾರೆ: ಗುರುತಿನ ಪರಿಶೀಲನೆಯ ಕೊರತೆ, ಸೀಮಿತ ಡಿಜಿಟಲ್ ಸಾಕ್ಷರತೆ, ಉತ್ಪನ್ನ ದಾಖಲಾತಿ ಕೌಶಲ್ಯದ ಕೊರತೆ ಮತ್ತು ಇ-ಕಾಮರ್ಸ್ ವೇದಿಕೆಗಳನ್ನು ನ್ಯಾವಿಗೇಟ್ ಮಾಡಲು ಕಷ್ಟವಾಗುವುದು.",
      solution: "ನಮ್ಮ ಪರಿಹಾರ",
      solutionDesc: "ನಾವು ಸಂಪೂರ್ಣ ಬೆಂಬಲವನ್ನು ನೀಡುತ್ತೇವೆ: ಗುರುತು ಮತ್ತು ಕಲೆಯ ಪರಿಶೀಲನೆ, ಸಮಗ್ರ ತರಬೇತಿ, ಕ್ಯುರೇಟೆಡ್ ಪ್ರದರ್ಶನ ವೇದಿಕೆ ಮತ್ತು ಪ್ರಮುಖ ಮಾರುಕಟ್ಟೆಗಳಿಗೆ ತಯಾರು ಮಾಡಲು ಅಗತ್ಯವಾದ ಪರಿಕರಗಳು.",
      values: "ನಮ್ಮ ಮೌಲ್ಯಗಳು",
      respect: "ಗೌರವ",
      respectDesc: "ನಾವು ಸಾಂಪ್ರದಾಯಿಕ ಕೌಶಲ್ಯಗಳನ್ನು ಗೌರವಿಸುತ್ತೇವೆ ಮತ್ತು ಪ್ರತಿಯೊಬ್ಬ ಕುಶಲಕರ್ಮಿಯನ್ನು ಗೌರವ ಮತ್ತು ಘನತೆಯಿಂದ ನಡೆಸಿಕೊಳ್ಳುತ್ತೇವೆ.",
      simplicity: "ಸರಳತೆ",
      simplicityDesc: "ತಂತ್ರಜ್ಞಾನವು ಎಲ್ಲರಿಗೂ ಲಭ್ಯವಿರಬೇಕು. ನಾವು ವಿಷಯಗಳನ್ನು ಸರಳವಾಗಿ ಮತ್ತು ಅರ್ಥಮಾಡಿಕೊಳ್ಳಲು ಸುಲಭವಾಗಿ ಇರಿಸುತ್ತೇವೆ.",
      empowerment: "ಸಬಲೀಕರಣ",
      empowermentDesc: "ನಾವು ಕುಶಲಕರ್ಮಿಗಳಿಗೆ ಮಾರಾಟ ಮಾಡಲು ಮಾತ್ರ ಸಹಾಯ ಮಾಡುವುದಿಲ್ಲ - ಅವರು ಸ್ವತಂತ್ರವಾಗಿ ಅಭಿವೃದ್ಧಿ ಹೊಂದಲು ನಾವು ಕಲಿಸುತ್ತೇವೆ.",
    },
    shop: {
      title: "ಕುಶಲಕರ್ಮಿ ಮಾರುಕಟ್ಟೆ",
      subtitle: "ಅಧಿಕೃತ ಕರಕುಶಲ ಉತ್ಪನ್ನಗಳನ್ನು ಅನ್ವೇಷಿಸಿ",
      searchPlaceholder: "ಉತ್ಪನ್ನಗಳು ಅಥವಾ ಕುಶಲಕರ್ಮಿಗಳನ್ನು ಹುಡುಕಿ...",
      allCategories: "ಎಲ್ಲಾ",
      categories: {
        textiles: "ಬಟ್ಟೆಗಳು",
        pottery: "ಮಡಿಕೆ ಕೆಲಸ",
        woodwork: "ಮರಗೆಲಸ",
        metalwork: "ಲೋಹದ ಕೆಲಸ",
        jewelry: "ಆಭರಣ",
        other: "ಇತರ"
      },
      noResults: "ಯಾವುದೇ ಉತ್ಪನ್ನಗಳು ಕಂಡುಬಂದಿಲ್ಲ",
      noResultsDesc: "ನಿಮ್ಮ ಹುಡುಕಾಟ ಅಥವಾ ವರ್ಗದ ಫಿಲ್ಟರ್ ಅನ್ನು ಹೊಂದಿಸಲು ಪ್ರಯತ್ನಿಸಿ.",
      viewStory: "ಕಥೆಯ ನೋಡಿ"
    },
    common: {
      meetArtisans: "ನಮ್ಮ ಮಾಸ್ಟರ್ ಕುಶಲಕರ್ಮಿಗಳನ್ನು ಭೇಟಿ ಮಾಡಿ",
      discoverStories: "ದೇಶಾದ್ಯಂತದ ಸಾಂಪ್ರದಾಯಿಕ ಕುಶಲಕರ್ಮಿಗಳ ಕಥೆಗಳು ಮತ್ತು ಕೌಶಲ್ಯಗಳನ್ನು ಅನ್ವೇಷಿಸಿ.",
      exploreAll: "ಎಲ್ಲಾ ಕಲೆಗಳನ್ನು ಅನ್ವೇಷಿಸಿ",
      by: "ಇವರಿಂದ",
      inStock: "ದಾಸ್ತಾನು ಇದೆ",
      soldOut: "ಖಾಲಿಯಾಗಿದೆ"
    },
    dashboard: {
      welcome: "ನಮಸ್ಕಾರ",
      subtitle: "ನಿಮ್ಮ ಡಿಜಿಟಲ್ ಕುಶಲಕರ್ಮಿ ಪರಂಪರೆಯನ್ನು ನಿರ್ವಹಿಸಿ.",
      settings: "ಸೆಟ್ಟಿಂಗ್‌ಗಳು",
      viewPublicShop: "ಸಾರ್ವಜನಿಕ ಅಂಗಡಿಯನ್ನು ನೋಡಿ",
      verification: "ಪರಿಶೀಲನೆ",
      myProducts: "ನನ್ನ ಉತ್ಪನ್ನಗಳು",
      training: "ತರಬೇತಿ",
      startVerification: "ಪರಿಶೀಲನೆ ಪ್ರಾರಂಭಿಸಿ",
      addNewProduct: "ಹೊಸ ಉತ್ಪನ್ನವನ್ನು ಸೇರಿಸಿ",
      startLearning: "ಕಲಿಯಲು ಪ್ರಾರಂಭಿಸಿ",
      yourCollection: "ನಿಮ್ಮ ಸಂಗ್ರಹ",
      addAnother: "ಇನ್ನೊಂದನ್ನು ಸೇರಿಸಿ",
      editDetails: "ವಿವರಗಳನ್ನು ಸಂಪಾದಿಸಿ",
      pending: "ಬಾಕಿ ಇದೆ",
      active: "ಸಕ್ರಿಯ",
      new: "ಹೊಸ",
      noProducts: "ನೀವು ಇನ್ನೂ ಯಾವುದೇ ಉತ್ಪನ್ನಗಳನ್ನು ಸೇರಿಸಿಲ್ಲ.",
      launchFirst: "ನಿಮ್ಮ ಮೊದಲ ಉತ್ಪನ್ನವನ್ನು ಪ್ರಾರಂಭಿಸಿ",
      totalEarnings: "ಒಟ್ಟು ಗಳಿಕೆ",
      storeViews: "ಮಳಿಗೆಯ ವೀಕ್ಷಣೆಗಳು",
      activeProducts: "ಸಕ್ರಿಯ ಉತ್ಪನ್ನಗಳು",
      pendingItems: "ಬಾಕಿ ಇರುವ ಐಟಂಗಳು"
    },
    auth: {
      signIn: "ಸೈನ್ ಇನ್",
      welcomeBack: "ಮರಳಿ ಸ್ವಾಗತ",
      welcomeSubtitle: "ನಿಮ್ಮ ಕರಕುಶಲ, ನಿಮ್ಮ ಕಥೆ, ನಿಮ್ಮ ಮಾರುಕಟ್ಟೆ. ಕುಶಲಕರ್ಮಿ ಕನೆಕ್ಟ್‌ನೊಂದಿಗೆ ನಿಮ್ಮ ಪ್ರಯಾಣವನ್ನು ಮುಂದುವರಿಸಿ.",
      newTo: "ಕುಶಲಕರ್ಮಿ ಕನೆಕ್ಟ್‌ಗೆ ಹೊಸಬರೇ?",
      createAccount: "ಖಾತೆ ತೆರೆಯಿರಿ",
      email: "ಇಮೇಲ್ ವಿಳಾಸ",
      password: "ಪಾಸ್ವರ್ಡ್",
      forgotPassword: "ಪಾಸ್ವರ್ಡ್ ಮರೆತಿರಾ?",
      signingIn: "ಸೈನ್ ಇನ್ ಆಗುತ್ತಿದೆ...",
      placeholderEmail: "neeve@example.com",
      placeholderPassword: "••••••••",
      resetPassword: "ಪಾಸ್‌ವರ್ಡ್ ಮರುಹೊಂದಿಸಿ",
      sendResetLink: "ಮರುಹೊಂದಿಸುವ ಲಿಂಕ್ ಕಳುಹಿಸಿ",
      backToLogin: "ಲಾಗಿನ್‌ಗೆ ಹಿಂತಿರುಗಿ",
      updatePassword: "ಪಾಸ್‌ವರ್ಡ್ ನವೀಕರಿಸಿ",
      newPassword: "ಹೊಸ ಪಾಸ್‌ವರ್ಡ್",
      confirmPassword: "ಪಾಸ್‌ವರ್ಡ್ ದೃಢೀಕರಿಸಿ",
      passwordUpdated: "ಪಾಸ್‌ವರ್ಡ್ ಯಶಸ್ವಿಯಾಗಿ ನವೀಕರಿಸಲಾಗಿದೆ!",
      checkEmail: "ಮುಂದುವರಿಯಿರಿ ಲಿಂಕ್‌ಗಾಗಿ ನಿಮ್ಮ ಇಮೇಲ್ ಪರಿಶೀಲಿಸಿ."
    },
    checkout: {
      title: "ಚೆಕ್‌ಔಟ್",
      shippingDetails: "ಶಿಪ್ಪಿಂಗ್ ವಿವರಗಳು",
      fullName: " ಪೂರ್ಣ ಹೆಸರು",
      phone: "ದೂರವಾಣಿ ಸಂಖ್ಯೆ",
      address: "ವಿಳಾಸ",
      city: "ನಗರ",
      state: "ರಾಜ್ಯ",
      pincode: "ಪಿನ್‌ಕೋಡ್",
      landmark: "ಲ್ಯಾಂಡ್‌ಮಾರ್ಕ್ (ಐಚ್ಛಿಕ)",
      checkDelivery: "ವಿತರಣೆಯನ್ನು ಪರಿಶೀಲಿಸಿ",
      placeOrder: "ಆರ್ಡರ್ ಮಾಡಿ",
      deliveryAvailable: "ಈ ಸ್ಥಳಕ್ಕೆ ವಿತರಣೆ ಲಭ್ಯವಿದೆ!",
      deliveryUnavailable: "ಕ್ಷಮಿಸಿ, ನಾವು ಈ ಸ್ಥಳಕ್ಕೆ ಇನ್ನೂ ಶಿಪ್ಪಿಂಗ್ ಮಾಡುವುದಿಲ್ಲ.",
      orderSuccess: "ಆರ್ಡರ್ ಯಶಸ್ವಿಯಾಗಿ ಇರಿಸಲಾಗಿದೆ!",
      orderSuccessDesc: "ಸಾಂಪ್ರದಾಯಿಕ ಕುಶಲಕರ್ಮಿಗಳನ್ನು ಬೆಂಬಲಿಸಿದ್ದಕ್ಕಾಗಿ ಧನ್ಯವಾದಗಳು.",
      backToHome: "ಮನೆಗೆ ಹಿಂತಿರುಗಿ",
      checking: "ಪರಿಶೀಲಿಸಲಾಗುತ್ತಿದೆ...",
      processing: "ಪ್ರಕ್ರಿಯೆಗೊಳಿಸಲಾಗುತ್ತಿದೆ...",
      total: "ಪಾವತಿಸಬೇಕಾದ ಒಟ್ಟು",
      secure: "ಸುರಕ್ಷಿತ ಪಾವತಿ",
      secureCheckout: "ಸುರಕ್ಷಿತ ಚೆಕ್‌ಔಟ್",
      verified: "ಪರಿಶೀಲಿಸಿದ ಕುಶಲಕರ್ಮಿಗಳು",
      orderSummary: "ಆರ್ಡರ್ ಸಾರಾಂಶ",
      subtotal: "ಉಪಮೊತ್ತ",
      shipping: "ಶಿಪ್ಪಿಂಗ್",
      free: "ಉಚಿತ",
    },
    cart: {
      title: "ನಿಮ್ಮ ಕಾರ್ಟ್",
      empty: "ನಿಮ್ಮ ಕಾರ್ಟ್ ಖಾಲಿಯಾಗಿದೆ",
      emptyDesc: "ನಮ್ಮ ಕರಕುಶಲ ಸಂಪತ್ತನ್ನು ಅನ್ವೇಷಿಸಿ",
      continueShopping: "ಶಾಪೇಂಗ್ ಮುಂದುವರಿಸಿ",
      proceed: "ಚೆಕ್‌ಔಟ್‌ಗೆ ಮುಂದುವರಿಯಿರಿ",
    },
    training: {
      title: "ಕುಶಲಕರ್ಮಿ ಅಕಾಡೆಮಿ",
      subtitle: "ಡಿಜಿಟಲ್ ಮಾರುಕಟ್ಟೆಯಲ್ಲಿ ಯಶಸ್ವಿಯಾಗಲು ಅಗತ್ಯವಿರುವ ಕೌಶಲ್ಯಗಳನ್ನು ಕರಗತ ಮಾಡಿಕೊಳ್ಳಿ.",
      backToDashboard: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್‌ಗೆ ಹಿಂತಿರುಗಿ",
      startModule: "ತರಬೇತಿ ಪ್ರಾರಂಭಿಸಿ",
      certification: "ಕುಶಲಕರ್ಮಿ ಪ್ರಮಾಣೀಕರಣ",
      certSubtitle: "ನಿಮ್ಮ ಸಾರ್ವಜನಿಕ ಪ್ರೊಫೈಲ್‌ನಲ್ಲಿ 'ಡಿಜಿಟಲ್ ರೆಡಿ' ಬ್ಯಾಡ್ಜ್ ಪಡೆಯಲು ಎಲ್ಲಾ ಮೂಲಭೂತ ತರಬೇತಿಗಳನ್ನು ಪೂರ್ಣಗೊಳಿಸಿ.",
      modulesComplete: "ತರಬೇತಿಗಳು ಪೂರ್ಣಗೊಂಡಿವೆ",
      read: "ಓದಿ",
      modules: {
        "pricing-strategy": {
          title: "ಡಿಜಿಟಲ್ ಬೆಲೆ ತಂತ್ರ",
          description: "ವೆಚ್ಚಗಳನ್ನು ಲೆಕ್ಕಹಾಕುವುದು, ತೆರಿಗೆಗಳನ್ನು ನಿರ್ವಹಿಸುವುದು ಮತ್ತು ಅಂತರರಾಷ್ಟ್ರೀಯ ಗ್ರಾಹಕರಿಗೆ ಸ್ಪರ್ಧಾತ್ಮಕ ಬೆಲೆಗಳನ್ನು ನಿಗದಿಪಡಿಸುವುದನ್ನು ಕಲಿಯಿರಿ.",
          level: "ಅಗತ್ಯ",
          duration: "10 ನಿಮಿಷ",
          content: "ಆನ್‌ಲೈನ್‌ನಲ್ಲಿ ಮಾರಾಟ ಮಾಡುವುದು ಸ್ಥಳೀಯ ಸಂತೆಗಳಲ್ಲಿ ಮಾರಾಟ ಮಾಡುವುದಕ್ಕಿಂತ ಭಿನ್ನವಾಗಿದೆ. ನೀವು ಗುಪ್ತ ವೆಚ್ಚಗಳನ್ನು ಲೆಕ್ಕಹಾಕಬೇಕು.\n\n### ಬೆಲೆ ಸೂತ್ರ\n**ಬೆಲೆ = (ವಸ್ತುಗಳು + ಶ್ರಮ + ಮೇುವೆಚ್ಚ) × ಲಾಭಾಂಶ + ಶಿಪ್ಪಿಂಗ್ + ತೆರಿಗೆಗಳು**\n\n#### 1. ನಿಮ್ಮ ವೆಚ್ಚಗಳನ್ನು ತಿಳಿಯಿರಿ\nಕೇವಲ ಬಟ್ಟೆ ಅಥವಾ ಮಣ್ಣನ್ನು ಲೆಕ್ಕಿಸಬೇಡಿ. ವಿದ್ಯುತ್, ನಿಮ್ಮ ಪ್ಯಾಕೇಜಿಂಗ್ ಸಮಯ ಮತ್ತು ನೀವು ಕುಡಿದ ಚಹಾವನ್ನು ಕೂಡ ಲೆಕ್ಕಿಸಿ!\n\n#### 2. ಜಾಗತಿಕ ಸನ್ನಿವೇಶ\nಯುರೋಪ್ ಅಥವಾ ಅಮೆರಿಕದ ಖರೀದಿದಾರರು ಮೊದಲು ಗುಣಮಟ್ಟವನ್ನು ನೋಡುತ್ತಾರೆ. ನಿಮ್ಮ ಕಠಿಣ ಪರಿಶ್ರಮದ ಬೆಲೆಯನ್ನು ಕಡಿಮೆ ಮಾಡಬೇಡಿ—ಇದು \"ಅಮೂಲ್ಯ\" ಎನ್ನುವ ಬದಲು \"ಅಗ್ಗ\" ಎಂದು ತೋರುತ್ತದೆ.\n\n#### 3. ಪ್ಲಾಟ್‌ಫಾರ್ಮ್ ಶುಲ್ಕಗಳು\nಮಾರುಕಟ್ಟೆಗಳು ಸಣ್ಣ ಪಾಲನ್ನು ತೆಗೆದುಕೊಳ್ಳುತ್ತವೆ ಎಂಬುದನ್ನು ನೆನಪಿಡಿ (ಸಾಮಾನ್ಯವಾಗಿ 5-15%). ಇದನ್ನು ನಿಮ್ಮ ಅಂತಿಮ ಬೆಲೆಯಲ್ಲಿ ಸೇರಿಸಿ.",
          tips: ["ಹತಾಶೆಯ ಆಧಾರದ ಮೇಲೆ ಎಂದಿಗೂ ಬೆಲೆ ನಿಗದಿಪಡಿಸಬೇಡಿ.", "ಅದೇ ರೀತಿಯ ಪ್ರೀಮಿಯಂ ಉತ್ಪನ್ನಗಳನ್ನು ಸಂಶೋಧಿಸಿ.", "ಬೆಲೆಯಲ್ಲಿ ಸೇರಿಸುವ ಮೂಲಕ ಉಚಿತ ಶಿಪ್ಪಿಂಗ್ ನೀಡಿ."]
        },
        "product-photography": {
          title: "ಕರಕುಶಲ ಛಾಯಾಗ್ರಹಣ",
          description: "ನಿಮ್ಮ ಸ್ಮಾರ್ಟ್‌ಫೋನ್ ಮತ್ತು ನೈಸರ್ಗಿಕ ಬೆಳಕನ್ನು ಬಳಸಿ ನಿಮ್ಮ ಕಲೆಯ ಆತ್ಮವನ್ನು ಸೆರೆಹಿಡಿಯಿರಿ.",
          level: "ಮಧ್ಯಮ",
          duration: "15 ನಿಮಿಷ",
          content: "ನಿಮ್ಮ ಫೋಟೋಗಳು ನಿಮ್ಮ ಗ್ರಾಹಕರು ಖರೀದಿಸುವ ಮೊದಲು ಸ್ಪರ್ಶಿಸಬಹುದಾದ ಏಕೈಕ ವಿಷಯವಾಗಿದೆ.\n\n### ಮೂರು ಸ್ತಂಭಗಳು\n\n#### 1. ಬೆಳಕೇ ಎಲ್ಲವೂ\nನೈಸರ್ಗಿಕ ಕಿಟಕಿ ಬೆಳಕನ್ನು ಬಳಸಿ. ನೇರ ಬಿಸಿಲನ್ನು ತಪ್ಪಿಸಿ. ಮುಂಜಾನೆ ಅಥವಾ ಸಂಜೆಯ ಬೆಳಕು \"ಚಿನ್ನದಂತಿರುತ್ತದೆ\".\n\n#### 2. ಸ್ವಚ್ಛ ಹಿನ್ನೆಲೆಗಳು\nತಿಳಿ ಮರ, ಕಲ್ಲು ಅಥವಾ ಬಿಳಿ ಲಿನಿನ್‌ನಂತಹ ಮಣ್ಣಿನ, ತಟಸ್ಥ ಹಿನ್ನೆಲೆಗಳು ಸಾಂಪ್ರದಾಯಿಕ ಕಲೆಗಳಿಗೆ ಉತ್ತಮವಾಗಿ ಕಾರ್ಯನಿರ್ವಹಿಸುತ್ತವೆ.\n\n#### 3. ವಿವರವಾದ ಶಾಟ್\nಖರೀದಿದಾರರು ನೇಯ್ಗೆ, ಕುಂಚದ ಹೊಡೆತ ಮತ್ತು ವಿನ್ಯಾಸವನ್ನು ನೋಡಲು ಬಯಸುತ್ತಾರೆ. ಕನಿಷ್ಠ ಒಂದು ಕ್ಲೋಸ್-ಅಪ್ ಮ್ಯಾಕ್ರೋ ಶಾಟ್ ತೆಗೆದುಕೊಳ್ಳಿ.",
          tips: ["ನಿಮ್ಮ ಫೋನ್ ಲೆನ್ಸ್ ಸ್ವಚ್ಛಗೊಳಿಸಿ.", "ಟ್ರೈಪಾಡ್ (ಅಥವಾ ಪುಸ್ತಕಗಳ ರಾಶಿ) ಬಳಸಿ.", "ಯಾವುದೇ ಆಕರ್ಷಕ ಫಿಲ್ಟರ್‌ಗಳಿಲ್ಲ—ಅದನ್ನು ನೈಜವಾಗಿಡಿ."]
        },
        "digital-storytelling": {
          title: "ಕಥೆ ಹೇಳುವ ಶಕ್ತಿ",
          description: "ಜಾಗತಿಕ ಖರೀದಿದಾರರೊಂದಿಗೆ ಭಾವನಾತ್ಮಕವಾಗಿ ಸಂಪರ್ಕ ಸಾಧಿಸಲು ಬಯೋ ಮತ್ತು ಉತ್ಪನ್ನ ಕಥೆಗಳನ್ನು ಬರೆಯುವುದು ಹೇಗೆ.",
          level: "ಮುಖ್ಯ",
          duration: "12 ನಿಮಿಷ",
          content: "ಜನರು ಕೇವಲ ಮಡಕೆ ಖರೀದಿಸುವುದಿಲ್ಲ; ಅವರು ಅದನ್ನು ಮಾಡಿದ ಕೈಗಳನ್ನು ಖರೀದಿಸುತ್ತಾರೆ.\n\n### ನಿಮ್ಮ ಕಥೆ ಮುಖ್ಯ\n\n#### 1. 'ಏಕೆ'\nನೀವು ಮಾಡುವುದನ್ನು ನೀವು ಏಕೆ ಮಾಡುತ್ತೀರಿ? ಇದು ಕೌಟುಂಬಿಕ ಸಂಪ್ರದಾಯವೇ? ನೀವು ನಂತರ ಕಂಡುಕೊಂಡ ಹವ್ಯಾಸವೇ?\n\n#### 2. ಪ್ರಕ್ರಿಯೆ\nಕಷ್ಟವನ್ನು ವಿವರಿಸಿ. \"ಒಂದು ಸೀರೆಯನ್ನು ನೇಯ್ಗೆ ಮಾಡಲು 40 ಗಂಟೆಗಳು ಬೇಕಾಗುತ್ತದೆ.\" ಇದು ಬೆಲೆಯನ್ನು ಸಮರ್ಥಿಸುತ್ತದೆ.\n\n#### 3. ಪರಿಣಾಮ\nನಿಮ್ಮಿಂದ ಖರೀದಿಸುವುದು ನಿಮ್ಮ ಹಳ್ಳಿಯನ್ನು, ನಿಮ್ಮ ಕುಟುಂಬವನ್ನು ಅಥವಾ ಸಾಯುತ್ತಿರುವ ಕಲೆಯನ್ನು ಬೆಂಬಲಿಸುತ್ತದೆ. ಅದನ್ನು ಹೆಮ್ಮೆಯಿಂದ ಹೇಳಿ.",
          tips: ["ನೀವು ಮಾತನಾಡುವಂತೆಯೇ ಬರೆಯಿರಿ.", "'ನಾನು' ಮತ್ತು 'ನನ್ನ' ಬಳಸಿ, 'ನಾವು' ಅಲ್ಲ (ಇದು ಸಹಕಾರ ಸಂಘವಲ್ಲದಿದ್ದರೆ).", "ನಿಮ್ಮ ವೈಫಲ್ಯಗಳನ್ನು ಸಹ ಹಂಚಿಕೊಳ್ಳಿ."]
        },
        "shipping-packaging": {
          title: "ಸುರಕ್ಷಿತ ಶಿಪ್ಪಿಂಗ್ ಮತ್ತು ಪ್ಯಾಕೇಜಿಂಗ್",
          description: "ಅನ್ಬಾಕ್ಸಿಂಗ್ ಅನುಭವಗಳನ್ನು ವಿನ್ಯಾಸಗೊಳಿಸಿ ನಿಮ್ಮ ಕಲೆಯನ್ನು ರಕ್ಷಿಸುವ ಮತ್ತು ನಿಮ್ಮ ಗ್ರಾಹಕರನ್ನು ಬೆರಗುಗೊಳಿಸುವ.",
          level: "ಪ್ರಾಯೋಗಿಕ",
          duration: "8 ನಿಮಿಷ",
          content: "ಮುರಿದ ಉತ್ಪನ್ನ = ಮುರಿದ ನಂಬಿಕೆ.\n\n### ಪ್ಯಾಕೇಜಿಂಗ್ ಮೂಲಗಳು\n\n#### 1. ಡ್ರಾಪ್ ಟೆಸ್ಟ್\nನಿಮ್ಮ ವಸ್ತುವನ್ನು ಪ್ಯಾಕ್ ಮಾಡಿ, ಅದನ್ನು ಸೀಲ್ ಮಾಡಿ ಮತ್ತು ನಡು ಎತ್ತರದಿಂದ ಕೆಳಕ್ಕೆ ಬಿಡಿ. ಅದು ಮುರಿದರೆ, ಅದನ್ನು ಉತ್ತಮವಾಗಿ ಪ್ಯಾಕ್ ಮಾಡಿ.\n\n#### 2. ಪರಿಸರ ಸ್ನೇಹಿ ಉತ್ತಮ\nಅಂತರರಾಷ್ಟ್ರೀಯ ಖರೀದಿದಾರರು ಸುಸ್ಥಿರ ಪ್ಯಾಕೇಜಿಂಗ್ ಅನ್ನು ಇಷ್ಟಪಡುತ್ತಾರೆ. ಸಾಧ್ಯವಾದರೆ ಬಬಲ್ ರಾಪ್ ಬದಲಿಗೆ ಪತ್ರಿಕೆ, ರಟ್ಟಿನ ಪೆಟ್ಟಿಗೆ ಅಥವಾ ಬಟ್ಟೆ ತುಂಡುಗಳನ್ನು ಬಳಸಿ.\n\n#### 3. ವೈಯಕ್ತಿಕ ಟಿಪ್ಪಣಿ\nಯಾವಾಗಲೂ ಕೈಬರಹದ ಧನ್ಯವಾದ ಟಿಪ್ಪಣಿಯನ್ನು ಸೇರಿಸಿ. ಇದು ಜೀವಿತಾವಧಿಯವರೆಗೆ ನಿಷ್ಠಾವಂತ ಗ್ರಾಹಕರನ್ನು ಸೃಷ್ಟಿಸುತ್ತದೆ.",
          tips: ["ನಾಜೂಕು ವಸ್ತುಗಳನ್ನು ಡಬಲ್-ಬಾಕ್ಸ್ ಮಾಡಿ.", "ಶಿಪ್ಪಿಂಗ್ ಮಾಡುವ ಮೊದಲು ಪ್ಯಾಕೇಜ್‌ನ ಫೋಟೋ ತೆಗೆದುಕೊಳ್ಳಿ.", "ನಿಮ್ಮ ಬಾಕ್ಸ್ ಅನ್ನು ಸ್ಟ್ಯಾಂಪ್‌ನೊಂದಿಗೆ ಬ್ರ್ಯಾಂಡ್ ಮಾಡಿ."]
        },
        "marketplace-verification": {
          title: "ಮಾರುಕಟ್ಟೆ ಪರಿಶೀಲನೆ (Amazon/Flipkart)",
          description: "ಪ್ರಮುಖ ಇ-ಕಾಮರ್ಸ್ ವೇದಿಕೆಗಳಲ್ಲಿ ಪರಿಶೀಲನೆ ಪಡೆಯಲು ಹಂತ-ಹಂತದ ಮಾರ್ಗದರ್ಶಿ.",
          level: "ಮುಂದುವರಿದ",
          duration: "20 ನಿಮಿಷ",
          content: "ದೊಡ್ಡದಾಗಿ ಬೆಳೆಯಲು ಸಿದ್ಧರಿದ್ದೀರಾ? Amazon Karigar ಮತ್ತು Flipkart Samarth ನಲ್ಲಿ ಹೇಗೆ ಸೇರಬೇಕೆಂಬುದು ಇಲ್ಲಿದೆ.\n\n### ನಿಮಗೆ ಬೇಕಾಗುವ ದಾಖಲೆಗಳು\n\n#### 1. GST ಸಂಖ್ಯೆ\nಆನ್‌ಲೈನ್ ಮಾರಾಟಕ್ಕೆ ಇದು ಕಡ್ಡಾಯವಾಗಿದೆ. ನಿಮ್ಮ ಬಳಿ ಇಲ್ಲದಿದ್ದರೆ, ಸರ್ಕಾರಿ ಪೋರ್ಟಲ್‌ನಲ್ಲಿ ಅರ್ಜಿ ಸಲ್ಲಿಸಿ. ಇದು ಉಚಿತವಾಗಿದೆ.\n\n#### 2. ಕುಶಲಕರ್ಮಿ ಕಾರ್ಡ್ / ಪೆಹಚಾನ್ ಕಾರ್ಡ್\nಈ ಸರ್ಕಾರಿ ಐಡಿ ನೀವು ಕಾನೂನುಬದ್ಧ ಕುಶಲಕರ್ಮಿ ಎಂದು ಸಾಬೀತುಪಡಿಸುತ್ತದೆ. ಇದು ಈ ಪ್ಲಾಟ್‌ಫಾರ್ಮ್‌ಗಳಲ್ಲಿ ನಿಮಗೆ ಕಡಿಮೆ ಆಯೋಗದ ದರಗಳನ್ನು ನೀಡುತ್ತದೆ.\n\n#### 3. ಬ್ಯಾಂಕ್ ಖಾತೆ\nನಿಮ್ಮ ಹೆಸರಿನಲ್ಲಿ ಅಥವಾ ನಿಮ್ಮ ವ್ಯಾಪಾರದ ಹೆಸರಿನಲ್ಲಿರಬೇಕು. ವ್ಯಾಪಾರೇತರ ಪಾಲುದಾರರೊಂದಿಗೆ ಜಂಟಿ ಖಾತೆಗಳಿಲ್ಲ.\n\n### ಪ್ರಕ್ರಿಯೆ\n1. Amazon Karigar ಅಥವಾ Flipkart Samarth ಮಾರಾಟಗಾರರ ಪೋರ್ಟಲ್‌ಗೆ ಹೋಗಿ.\n2. ನಿಮ್ಮ GST ಮತ್ತು ರದ್ದುಗೊಳಿಸಿದ ಚೆಕ್ ಅನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.\n3. 'ಪ್ರಮಾಣಪತ್ರಗಳು' ವಿಭಾಗದಲ್ಲಿ ನಿಮ್ಮ ಕುಶಲಕರ್ಮಿ ಕಾರ್ಡ್ ಅನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.\n4. ಅನುಮೋದನೆಗಾಗಿ 48-72 ಗಂಟೆಗಳ ಕಾಲ ಕಾಯಿರಿ.",
          tips: ["ನಿಮ್ಮ ಕುಶಲಕರ್ಮಿ ಕಾರ್ಡ್ ಅನ್ನು ನವೀಕರಿಸಿ.", "ಬ್ಯಾಂಕ್ ಖಾತೆಯಲ್ಲಿನ ಹೆಸರು GST ಗೆ ನಿಖರವಾಗಿ ಹೊಂದಿಕೆಯಾಗಿದೆಯೇ ಎಂದು ಖಚಿತಪಡಿಸಿಕೊಳ್ಳಿ.", "5-10 ಅತ್ಯುತ್ತಮ ಉತ್ಪನ್ನಗಳೊಂದಿಗೆ ಪ್ರಾರಂಭಿಸಿ."]
        }
      }
    },
    home: {
      artisanShowcase: {
        title: "ನಮ್ಮ ಮಾಸ್ಟರ್ ಕುಶಲಕರ್ಮಿಗಳನ್ನು ಭೇಟಿ ಮಾಡಿ",
        subtitle: "ದೇಶಾದ್ಯಂತದ ಸಾಂಪ್ರದಾಯಿಕ ಕುಶಲಕರ್ಮಿಗಳ ಕಥೆಗಳು ಮತ್ತು ಕೌಶಲ್ಯಗಳನ್ನು ಅನ್ವೇಷಿಸಿ.",
        explore: "ಎಲ್ಲಾ ಕುಶಲಕರ್ಮಿಗಳನ್ನು ಅನ್ವೇಷಿಸಿ"
      }
    },
    reviews: {
      title: "ಗ್ರಾಹಕ ವಿಮರ್ಶೆಗಳು",
      writeReview: "ವಿಮರ್ಶೆ ಬರೆಯಿರಿ",
      yourReview: "ನಿಮ್ಮ ವಿಮರ್ಶೆ",
      noReviews: "ಇನ್ನೂ ಯಾವುದೇ ವಿಮರ್ಶೆಗಳಿಲ್ಲ. ಮೊದಲ ವಿಮರ್ಶಕರಾಗಿ!",
      loginPrompt: "ವಿಮರ್ಶೆ ನೀಡಲು ಲಾಗಿನ್ ಮಾಡಿ"
    },
    favorites: {
      title: "ನಿಮ್ಮ ಮೆಚ್ಚಿನವುಗಳು",
      noFavorites: "ಇನ್ನೂ ಮೆಚ್ಚಿನವುಗಳಿಲ್ಲ",
      addedTo: "ಮೆಚ್ಚಿನವುಗಳಿಗೆ ಸೇರಿಸಲಾಗಿದೆ",
      removedFrom: "ಮೆಚ್ಚಿನವುಗಳಿಂದ ತೆಗೆದುಹಾಕಲಾಗಿದೆ"
    },
    contact: {
      title: "ಕುಶಲಕರ್ಮಿಯನ್ನು ಸಂಪರ್ಕಿಸಿ",
      button: "ಸಂಪರ್ಕಿಸಿ",
      sendTo: "ಸಂದೇಶ ಕಳುಹಿಸಿ",
      subject: "ವಿಷಯ",
      message: "ಸಂದೇಶ",
      send: "ಸಂದೇಶ ಕಳುಹಿಸಿ",
      cancel: "ರದ್ದುಮಾಡಿ",
      success: "ಸಂದೇಶ ಕಳುಹಿಸಲಾಗಿದೆ!",
      successDesc: "ಕುಶಲಕರ್ಮಿಗೆ ಶೀಘ್ರದಲ್ಲೇ ನಿಮ್ಮ ಸಂದೇಶ ಸಿಗುತ್ತದೆ.",
      loginRequired: "ಸಂದೇಶ ಕಳುಹಿಸಲು ಲಾಗಿನ್ ಮಾಡಿ"
    },
    gallery: {
      title: "ಹಿಂದಿನ ಕಥೆ",
      subtitle: "ಸೃಜನಾತ್ಮಕ ಪ್ರಕ್ರಿಯೆಯ ಒಂದು ನೋಟ",
      manage: "ಹಿಂದಿನ ಕಥೆ ಗ್ಯಾಲರಿ"
    },
    messages: {
      inbox: "ಸಂದೇಶಗಳು",
      noMessages: "ಇನ್ನೂ ಯಾವುದೇ ಸಂದೇಶಗಳಿಲ್ಲ"
    },
    verification_page: {
      title: "ನಿಮ್ಮ ಕಲೆಯನ್ನು ಪರಿಶೀಲಿಸಿ",
      whyTitle: "ನಮಗೆ ಇದು ಏಕೆ ಬೇಕು?",
      whyDesc: "ಕುಶಲಕರ್ಮಿ ಕನೆಕ್ಟ್‌ನಲ್ಲಿರುವ ಪ್ರತಿಯೊಂದು ಉತ್ಪನ್ನವು ನಿಮ್ಮಂತಹ ನಿಜವಾದ ಕುಶಲಕರ್ಮಿಗಳಿಂದ 100% ಕೈಯಿಂದ ತಯಾರಿಸಲ್ಪಟ್ಟಿದೆ ಎಂದು ನಾವು ಖಚಿತಪಡಿಸಿಕೊಳ್ಳಲು ಬಯಸುತ್ತೇವೆ. ಇದು ನಿಮ್ಮ ಕಠಿಣ ಪರಿಶ್ರಮವನ್ನು ಮರುಮಾರಾಟಗಾರರಿಂದ ರಕ್ಷಿಸುತ್ತದೆ.",
      step1Title: "1. \" ಕೆಲಸದ ಪುರಾವೆ\" ಸೆಲ್ಫಿ",
      step1Desc: "ವಂಚಕರನ್ನು ತಡೆಯಲು, ನಿಮ್ಮ ಕಲೆಯೊಂದಿಗೆ ನಿಮ್ಮನ್ನು ನಾವು ನೋಡಬೇಕು.",
      takePhoto: "ಸೆಲ್ಫಿ ತೆಗೆದುಕೊಳ್ಳಿ",
      clickUpload: "ಅಪ್‌ಲೋಡ್ ಮಾಡಲು ಅಥವಾ ಫೋಟೋ ತೆಗೆದುಕೊಳ್ಳಲು ಕ್ಲಿಕ್ ಮಾಡಿ",
      step2Title: "2. ಗುರುತಿನ ಚೀಟಿ",
      step2Desc: "ನಿಮ್ಮ ಕುಶಲಕರ್ಮಿ ಕಾರ್ಡ್, ಆಧಾರ್ ಅಥವಾ ಸರ್ಕಾರಿ ಗುರುತಿನ ಚೀಟಿಯನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
      uploadId: "ಐಡಿ ಪ್ರೂಫ್ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
      uploadDoc: "ದಾಖಲೆಯನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ (JPG, PNG, PDF)",
      step3Title: "3. ಕೆಲಸದ ಮಾದರಿಗಳು",
      step3Desc: "ನಿಮ್ಮ ಇತ್ತೀಚಿನ ಕೆಲಸದ 2-3 ಫೋಟೋಗಳನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
      uploadSamples: "ಕೆಲಸದ ಮಾದರಿಗಳನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
      selectPhotos: "ಫೋಟೋಗಳನ್ನು ಆಯ್ಕೆಮಾಡಿ",
      filesSelected: "ಫೈಲ್‌ಗಳನ್ನು ಆಯ್ಕೆಮಾಡಲಾಗಿದೆ",
      step4Title: "4. ವ್ಯಾಪಾರ ಮತ್ತು ಬ್ಯಾಂಕಿಂಗ್",
      step4Desc: "ನಿಮ್ಮ ಖಾತೆಗೆ ನೇರವಾಗಿ ಪಾವತಿಗಳನ್ನು ಸ್ವೀಕರಿಸಲು ಅಗತ್ಯವಿದೆ.",
      labelGST: "GST ಸಂಖ್ಯೆ (ಐಚ್ಛಿಕ)",
      labelAccountName: "ಖಾತೆದಾರರ ಹೆಸರು",
      labelAccountNumber: "ಬ್ಯಾಂಕ್ ಖಾತೆ ಸಂಖ್ಯೆ",
      labelIFSC: "IFSC ಕೋಡ್",
      labelBankProof: "ರದ್ದುಗೊಳಿಸಿದ ಚೆಕ್ / ಪಾಸ್‌ಬುಕ್",
      uploadBankProof: "ಬ್ಯಾಂಕ್ ಪುರಾವೆ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
      uploading: "ಅಪ್‌ಲೋಡ್ ಆಗುತ್ತಿದೆ...",
      submit: "ಪರಿಶೀಲನೆಯನ್ನು ಸಲ್ಲಿಸಿ",
      success: "ಪರಿಶೀಲನೆಯನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಸಲ್ಲಿಸಲಾಗಿದೆ!",
      alertId: "ದಯವಿಟ್ಟು ಐಡಿ ಪ್ರೂಫ್ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
      alertSelfie: "ದಯವಿಟ್ಟು ಸೆಲ್ಫಿ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
      alertBank: "ದಯವಿಟ್ಟು ಬ್ಯಾಂಕಿಂಗ್ ವಿವರಗಳನ್ನು ಪೂರ್ಣಗೊಳಿಸಿ"
    },
    admin: {
      title: "ನಿರ್ವಾಹಕ ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
      pendingRequests: "ಬಾಕಿ ಉಳಿದಿರುವ ಪರಿಶೀಲನೆಗಳು",
      noRequests: "ಯಾವುದೇ ಬಾಕಿ ವಿನಂತಿಗಳಿಲ್ಲ.",
      approve: "ಅನುಮೋದಿಸಿ",
      reject: "ತಿರಸ್ಕರಿಸಿ",
      viewProof: "ಪುರಾವೆಗಳನ್ನು ವೀಕ್ಷಿಸಿ",
      gst: "GST",
      bank: "ಬ್ಯಾಂಕಿಂಗ್",
      status: "ಸ್ಥಿತಿ",
      action: "ಕ್ರಿಯೆ"
    }
  }
};
